...
把这个项目从一次性 Cursor SDK 验证，推进成一个长期只服务 Cursor 的 remote-control server/client 体验：一台有本地 repo 的机器上运行小型 server，浏览器 UI 像 OpenCode client 一样远程控制 Cursor agent conversation。Server 负责 Cursor API key、SDK 生命周期、本地 repo 路径、事件流和 session 历史；Web client 负责选择 conversation、发送 prompt、渲染 chat timeline、展示 thinking/tool activity，并让用户持续观察 agent 在指定 cwd 里写代码。

这个项目已经证明 `@cursor/sdk` 可以被 Node 后端调用，也已经通过 live integration 证明真实 token + local cwd + app SSE 能跑通。下一步不是继续把它做成 benchmark/event monitor，而是把前端改成真正的 coding chat client：左侧 conversations，右侧 chat window，底部 composer，消息流里直接渲染 assistant text、thinking、tool calls、status 和 errors。

## 用户

初始用户是已经有 OpenCode iOS client 经验的 agent 工具构建者。他理解 OpenCode 的 server/client 模式，希望用 Cursor 做一个相似但更窄的产品形态：从笔记本、手机或平板打开网页，选择一个 conversation，输入任务，看 Cursor agent 在配置好的 repo 里工作，并在同一个 chat timeline 里看到回复、thinking/activity、工具调用和最终状态。
...
当前 Cursor POC 已经从阻塞式 `POST /api/runs` 进化到 session/run/SSE，但前端仍然像一个 launcher 加 event monitor。它能证明传输链路，却还不能证明真正的产品体验：用户需要进入某个 conversation，像 chat client 一样发任务，看到 agent reply、tool cards、thinking/status 和 run lifecycle 自然组成一条 timeline。

## 需求

### Stage 1 必须有

- React web client，产品形态是 OpenCode-like coding chat client：左侧 conversation/session list，右侧 chat timeline，底部 prompt composer。
...
- Live Cursor evaluation 是一等需求：当 `RUN_CURSOR_LIVE_TESTS=1` 且 `.env` 里有 `CURSOR_API_KEY` 时，测试应能用 `composer-2` 在一次性 sandbox cwd 中验证真实 SDK 的 stream、文件写入和 SSE 转发。
...
第一，REST 负责 durable state，例如 sessions、messages、status 和 run metadata。第二，SSE 负责实时更新，例如 connected、heartbeat、session status、message/part updates、tool progress。第三，client 把 agent 工作渲染成 timeline：text、reasoning/activity、tool cards、task/status 和 final result。
...
2. Mock mode 可以创建 conversation、发送 prompt、stream synthetic events，并在刷新后恢复 chat timeline。
3. Local Cursor mode 可以在 `CURSOR_LOCAL_CWD` 上启动 run，同时不把 API key 暴露给浏览器。
4. 浏览器可以看到 live run activity，而不是等 `run.wait()` 完成。
5. Chat timeline 能区分 user prompt、assistant text、thinking/activity、tool call、status change、final result 和 error；raw event names 不出现在默认主界面。
...
8. Live Cursor suite 在有 token 时可以客观判断 API 是否可用：`composer-2` local run 可用、sandbox 文件被写入、SSE 收到完整 run event、event replay 不丢事件。
9. README 清楚解释 `.env` 配置、`HOST=0.0.0.0` 的 Tailscale/LAN 暴露方式、默认 deterministic tests 和 opt-in live Cursor tests。
10. Playwright screenshot verifies the UI reflects the product shape: left conversations, right chat timeline, composer, and rendered activity/tool/status blocks. 之后再做 GLM design critique 和视觉 polish。
...
- Local runtime 没有 cloud artifacts 或 PR metadata；如果后续需要 diff/result review，应单独走本地 git 路径，不能阻塞当前 chat client。
