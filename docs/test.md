...
Live tests 显式 opt-in：

```bash
RUN_CURSOR_LIVE_TESTS=1 CURSOR_API_KEY=crsr_... npm test -- --run tests/live.cursor.test.ts
```

默认模型固定为 `composer-2`。用户会把 token 写进 `.env`；测试代码只读取环境变量，不依赖 1Password service account。Live tests 的 cwd 必须是一次性 sandbox，不允许指向真实 workspace。Sandbox 由测试创建和清理：
