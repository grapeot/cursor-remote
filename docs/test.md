...
Stage 1 的测试目标不是追求泛化 UI 覆盖率，而是让 agent 能自我判断这套 remote-control API 和 chat client 是否真的 work：HTTP route 是否返回正确状态，SSE 是否按顺序产生 event，event log 是否可以 replay，Cursor stream 是否能映射成 app event，local sandbox 是否真的被修改，浏览器默认界面是否能把 user / assistant / synthetic **`thinking`** tool / 其它 tool / status 渲染成可用的 conversation timeline。
...
- `frontend/src/timeline.test.ts`：覆盖 frontend `TimelineItem` projection（含 synthetic **`thinking`** tool 与普通 tool 的合并）。
- `frontend/src/App.test.tsx`：在 happy-dom 中挂载 React app，用 mock API 与 mock EventSource 验证 conversation shell、composer 提交、`thinking`（synthetic tool 卡片）、其它 tool、`assistant`、`run.result` 与侧栏会话状态的一致性，以及在切换 session 后 tool 事件的缓存回放。
...
## 浏览器端到端（先于 Playwright）

与 Meta-Skill / T2 一致：**先定义可验证的最终状态**（UI 可用 + 磁盘 / 日志等客观产物），在真实 Chromium 上手跑通，再决定是否把同一套断言固化进 Playwright。过程记录见 **`docs/integration_wip.md`**。Playwright CI 暂不强制；等小步场景稳定后再加。

- **辅助样例**：`tests/fixtures/two_sum/` + `tests/two_sum_harness.node.test.ts`，用 **`python3 -m unittest discover`**（无 pytest 依赖），证明「小题 + 自检脚本」范式可独立跑通，与 Cursor 运行时解耦。
...
| Cursor stream mapper | fixture：`assistant`、`tool`、`status`、`task`、`request`、未知 shape；`**thinking`** 仅在 gateway 聚合层测试（`thinkingFlush`/`executeRun` 路径），mapper 对其返回空数组 | unknown 不 crash；payload envelope 稳定 |
...
| Frontend projection | event fixtures → session list、timeline、status badge、synthetic **`thinking`** tool + 其它 assistant/tool blocks | UI projection 纯函数可测 |
| Frontend DOM flow | mock health/session/run API + mock EventSource → composer submit → SSE 驱动的 timeline；**侧栏会话状态与乐观更新/`run.status` 一致** | user prompt、`thinking`/`write_file` tool 卡片、assistant、stream close 与 Conversations 列表状态均可 DOM 断言 |
...
默认模型固定为 `composer-2`。用户在 `.env` 中配置 token；测试只读环境变量。产品默认可把 `CURSOR_LOCAL_CWD=.` 指向本仓库根用于自举；Live tests 的 cwd 必须是一次性 sandbox，不得指向线上工作副本。Sandbox 由测试创建和清理：
