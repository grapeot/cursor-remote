...
- `frontend/src/App.test.tsx`：在 happy-dom 中挂载 React app，用 mock API 和 mock EventSource 覆盖默认 conversation shell、composer submit、streamed thinking/tool/assistant/result 渲染，以及 `run.result` 后关闭 event stream。
...
| Frontend DOM flow | mock health/session/run API + mock EventSource -> composer submit -> streamed events render in conversation timeline | user prompt、assistant result、thinking、tool card、stream close 都在 DOM 中可断言 |
